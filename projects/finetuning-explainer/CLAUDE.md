# Facet Video Production Workspace: Fine-Tuning LLMs Explainer
...
- `explainer` (`.claude/skills/explainer/SKILL.md`)

## Production Workflow
1. **Intake & Brief:** Read script.md and remotion_props.json.
2. **Narration:** Write timed narrative beats and synthesize audio with:
   facet tools run edgetts --input artifacts/req_tts.json
3. **Probe Duration:** Verify audio lengths with:
   facet tools run media_probe --input '{"file_path": "narration/s1.mp3"}'
4. **Scene Plan & Edit:** Construct visual cards/components in remotion_props.json.
5. **Render & QA:** Review the output using:
