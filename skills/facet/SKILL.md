...
gflow authenticates through a Chrome extension and a local bridge, so `gflow status` reporting healthy is necessary but not sufficient — a generation can still fail with `CAPTCHA_FAILED: Cannot access contents of the page. Extension manifest must request permission to access the respective host` while status shows the token captured and the extension connected. That is the extension lacking host permission for the page it needs to access, not a credential problem: re-check the extension's site access in Chrome rather than the API key. Facet reports the provider's own error line, so read that rather than assuming authentication.
