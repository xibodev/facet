...
(async()=>{
 ...
 for(const file of tracked){
  if(!/^(skills|packs|agents|schemas|styles|pipeline_defs)\//.test(file)&&!/^remotion-composer\/(src\/|package(?:-lock)?\.json$|tsconfig\.json$)/.test(file))continue;
 ...
 for(const file of ['LICENSE','THIRD_PARTY_NOTICES.md','PROVENANCE.md'])await fs.copyFile(path.join(repo,file),path.join(stage,file));
